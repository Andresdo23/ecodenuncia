...
const API_URL = 'http://192.168.0.5:3001/api'; // <-- O seu IP
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          <TextInput style={styles.input} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" placeholder="seu@email.com" />
...
const CadastroScreen = ({ onNavigateLogin, onRegisterSuccess }) => {
  ...
  const handleRegister = async () => {
    setError(''); 
    if (!nome || !email || !senha || !confirmarSenha || !cpf || !telefone || !dataNascimento) { 
      setError('Todos os campos marcados com * são obrigatórios.'); return; }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAvoidingView 
        ...
        style={styles.keyboardAvoidingContainer}
      >
        <ScrollView style={styles.scrollHack} contentContainerStyle={styles.scrollContainer}>
          ...
          <TextInput style={styles.input} value={cpf} onChangeText={setCpf} keyboardType="numeric" placeholder="111.222.333-44" />
          <Text style={styles.label}>Telefone *</Text>
          <TextInput style={styles.input} value={telefone} onChangeText={setTelefone} keyboardType="phone-pad" placeholder="(XX) 9XXXX-XXXX" />
          <Text style={styles.label}>Data de Nascimento *</Text>
          <TextInput style={styles.input} value={dataNascimento} onChangeText={setDataNascimento} placeholder="DD/MM/AAAA" />
          <Text style={styles.label}>Senha *</Text>
          <View style={styles.passwordContainer}>
            <TextInput style={styles.inputPassword} value={senha} onChangeText={setSenha} secureTextEntry={!mostrarSenha} placeholder="Mínimo 6 caracteres" />
            <TouchableOpacity onPress={() => setMostrarSenha(!mostrarSenha)} style={styles.eyeButton}>
              <Text style={styles.eyeButtonText}>{mostrarSenha ? 'Ocultar' : 'Ver'}</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.passwordContainer}>
            <TextInput style={styles.inputPassword} value={confirmarSenha} onChangeText={setConfirmarSenha} secureTextEntry={!mostrarSenha} placeholder="Repita a senha" />
          </View>
          {error ? <Text style={styles.error}>{error}</Text> : null}
      ...
      <Modal transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={[styles.input, { marginHorizontal: 0 }]} placeholder="Motivo (ex: duplicada)" value={motivoExclusao} onChangeText={setMotivoExclusao} />
...
const NovaDenunciaScreen = ({ onBack, onDenunciaCriada }) => {
  ...
  const handleSubmit = async () => {
    if (!imagem || !descricao || !endereco) { 
      Alert.alert("Campos Obrigatórios", "Descrição, Endereço e Foto são obrigatórios para enviar a denúncia."); 
      return; 
    }
    ...
    try {
      ...
      await api.post('/denuncias', {
        descricao, url_foto, latitude, longitude,
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScrollView style={styles.scrollHack} contentContainerStyle={styles.scrollContainer}>
          ...
          <TextInput style={styles.inputMulti} value={descricao} onChangeText={setDescricao} placeholder="Ex: Sofá abandonado na esquina." multiline />
          <Text style={styles.label}>Endereço Completo *</Text>
          <TextInput style={styles.input} value={endereco} onChangeText={setEndereco} placeholder="Ex: Rua das Flores, 123, Bairro Centro" />
          <Text style={styles.label}>Ponto de Referência (Opcional)</Text>
          <TextInput style={styles.input} value={referencia} onChangeText={setReferencia} placeholder="Ex: Perto da padaria da esquina" />
...
const EditDenunciaScreen = ({ onBack, onDenunciaEditada, denuncia }) => {
  ...
  const handleSubmit = async () => {
    if (!descricao || !endereco) { 
      Alert.alert("Erro", "Descrição e Endereço são obrigatórios."); 
      return; 
    }
    setIsSubmitting(true);
    try {
...
// --- 
// 7. TELA: EDITAR PERFIL (Atualizada com novas funcionalidades)
// ---
const EditProfileScreen = ({ onBack, onProfileUpdated, usuario, onNavigateChangePassword, onLogout }) => {
  ...
  const [mostrarSenhaExclusao, setMostrarSenhaExclusao] = useState(false); // <-- NOVO ESTADO
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      // O backend espera a senha no 'data' de um pedido DELETE
      await api.delete('/users/me', {
        data: { senha: senhaConfirm } 
      });
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalText}>Esta ação é permanente. Para confirmar, por favor, digite a sua senha:</Text>
            ...
            <TouchableOpacity 
              style={[styles.buttonPrimary, { backgroundColor: palette.danger, marginHorizontal: 0 }]} 
            ...
            <TouchableOpacity 
              style={[styles.buttonSecondary, { marginTop: 10, marginHorizontal: 0 }]} 
      ...
      >
        <ScrollView style={styles.scrollHack} contentContainerStyle={styles.scrollContainer}>
          ...
          <TouchableOpacity 
            style={[styles.buttonSecondary, { borderColor: palette.danger, marginTop: 20 }]} 
...
const ChangePasswordScreen = ({ onBack, onPasswordChanged }) => {
  ...
  // --- ALTERAÇÃO AQUI: Estados de "Ver/Ocultar" ---
...
export default function App() {
  ...
  // --- Adicionar o novo ecrã ao navegador ---
